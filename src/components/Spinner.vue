    ...
    <div v-if="!isDone">
        ...
        <div v-else
            ...
            <div class="checkmark w-5 h-5 flex items-center justify-center">
                <svg class=" fill-green-700" xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="phrase-container pl-2">
                <span :key="currentPhrase" class="phrase">Сервер запущен!</span>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    watch: {
        'isLoading': function (v) {
            if (v === false) {
                this.doSpin = false; // Stop the spinner when loading is complete
                setTimeout(() => {
                    this.isDone = true;
                }, 1900);
            }
    ...
    data() {
        return {
            isDone: false,
            doSpin: true, // This should be set to true when the backend is starting
            phrases: [
